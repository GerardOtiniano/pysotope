...
def _choose_x_star_overlap(df, area_col="area", group_col="chain"):
    mins = df.groupby(group_col)[area_col].min()
    maxs = df.groupby(group_col)[area_col].max()
    xmin_star = mins.max()
    xmax_star = maxs.min()
...
def build_norm_with_fallback(lin_std, y_col, area_col="area", chain_col="chain", group_col=None, name_col ="Identifier 1", log_file_path=None):
    """
    ...
    """
    group_col = chain_col if group_col is None else group_col
    df = lin_std[[name_col, area_col, group_col, y_col]].copy()
    x_star = _choose_x_star_overlap(df, area_col=area_col, group_col=group_col)
    if x_star is not None:
        ...
        for g, sub in df.groupby(group_col):
            ...
            if lvl is None or not np.isfinite(lvl):
                ...
                break
            levels[g] = float(lvl)
        if ok_all and len(levels) == df[group_col].nunique():
            R0 = float(np.median(list(levels.values())))
            norm = df.copy()
            norm[y_col] = norm.apply(lambda r: r[y_col] - levels[r[group_col]] + R0, axis=1)
    ...
    chain_means = df.groupby(group_col)[y_col].mean().to_dict()
    global_center = float(np.median(list(chain_means.values())))
    norm = df.copy()
    norm[y_col] = norm.apply(lambda r: r[y_col] - chain_means[r[group_col]] + global_center, axis=1)
