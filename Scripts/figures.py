...
def std_plot(lin, drift, folder_path, fig_path, isotope, cutoff_line=None, regress=False, dD="dD"):
    ...
    for i in [0, 2]:
        ...
        ax[i].text(0.9, 0.9, chain_length[i], horizontalalignment="center", verticalalignment="center", transform=ax[i].transAxes)

        temp = lin[lin.chain == chain_length[i + 1]]
        ax[i + 1].scatter(temp["area"], temp[dD], alpha=0.2, ec="k", s=80, c="orange")
        ax[i + 1].text(0.9, 0.9, chain_length[i + 1], horizontalalignment="center", verticalalignment="center", transform=ax[i + 1].transAxes)
...
def verify_lin_plot(lin, fig_path, dD_id, log_file_path, cutoff_line, isotope, regress=False):
    ...
    # slope, intercept, r_squared, p_value, std_err = robust_linear_regression(temp['area'], temp[dD_id])
    slope, intercept, r_squared, p_value, std_error, model = gls_regression(temp["area"], temp[dD_id], log_file_path)
    plt.plot([temp.area.min(), temp.area.max()], [temp.area.min() * slope + intercept, temp.area.max() * slope + intercept], c="k", linestyle="--")
    plt.legend(loc="upper center", bbox_to_anchor=(0.5, -0.15), fancybox=False, shadow=True, ncol=1)
    ...
    if num_chains > 1:
        ...
        for i, chain in enumerate(unique_chains):
            ...
            if "Final - Methanol Corrected " + str(isotope) in chain_unknown:
                axes[i].errorbar(chain_unknown["Mean Area"], chain_unknown["Final - Methanol Corrected " + str(isotope)], yerr=chain_unknown["Total Uncertainty"], linestyle="", fmt="", ecolor="red", alpha=0.5)
        ...
        plt.errorbar(unknown["Mean Area"], unknown["Final - Methanol Corrected " + str(isotope)], yerr=unknown["Total Uncertainty"], linestyle="", fmt="", ecolor="red", alpha=0.5)
